module.exports = function (nit, http, Self)
{
    return (Self = nit.defineClass ("http.Server"))
        ...
        .plugin ("lifecycle-component", "init", "start", "stop", "upgrade", { instancePluginAllowed: true })
        .plugin ("server")
        ...
        .onInitInvocationQueue (function (queue, self, method, args)
        {
            ...
            {
                queue.after (`${method}.invokeHook`, `${method}.callHost`, async function ()
                {
                    let [ctx] = args;

                    await ctx.host?.[method] (ctx);
                });
            }
            else
            {
                ...
                queue[order] (`${method}.invokeHook`, `${method}.callHosts`, async function ()
                {
        ...
        {
            queue.replace ("stop.callHosts", async function ()
            {
                try
                {
                    for (let host of self.hosts)
        ...
        .onConfigureQueueForDispatch (function (queue, self, args)
        {
            ...
            ctx.host = self.hosts.find (h => h.applicableTo (ctx));
        ...
        {
            queue.replace ("upgrade.callHosts", async function (req, socket, head)
            {
