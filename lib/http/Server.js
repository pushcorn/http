module.exports = function (nit, http, Self)
{
    ...
    return (Self = nit.defineClass ("http.Server"))
        ...
        .registerPlugin ("http.ServerPlugin", true)
        .plugin ("logger", { timestamp: true })
        .plugin ("event-emitter", "init", "start", "stop", "upgrade", "dispatch", { prePost: true })
        ...
        .defineInstanceDescriptor (Descriptor =>
        {
            Descriptor
                ...
                .field ("plugins...", "http.ServerPlugin.Descriptor")
                // assetResolvers and templateLoaders from plugins

                .onConfigure (function (server)
                {
                    let self = this;
                    let serverClass = server.constructor;

                    self.plugins.forEach (p => serverClass.serverplugin (p.build ()));
        ...
        .lifecycleMethod ("preStop", async function ()
        {
            ...
            await cls.applyPlugins ("serverplugins", "preStop", self);
            await self.emit ("preStop", self);
        })
        .lifecycleMethod ("postStop", async function ()
        {
            ...
            await cls.applyPlugins ("serverplugins", "postStop", self);
            await self.emit ("postStop", self);
        })
        ...
        .lifecycleMethod ("preDispatch", async function (ctx)
        {
            ...
            await cls.applyPlugins ("serverplugins", "preDispatch", self, ctx);
            await self.emit ("preDispatch", self, ctx);
        })
        .lifecycleMethod ("postDispatch", async function (ctx)
        {
            ...
            await cls.applyPlugins ("serverplugins", "postDispatch", self, ctx);
            await self.emit ("postDispatch", self, ctx);
        })
        ...
        .lifecycleMethod ("preInit", async function ()
        {
            ...
            await cls.applyPlugins ("serverplugins", "preInit", self);
            await self.emit ("preInit", self);
        })
        .lifecycleMethod ("postInit", async function ()
        {
            ...
            await cls.applyPlugins ("serverplugins", "postInit", self);
            await self.emit ("postInit", self);
        })
        .lifecycleMethod ("init", async function ()
        {
            let self = this;
            let cls = self.constructor;
            ...
            await cls[cls.kInit]?.call (self);
            await self.emit ("init", self);
        })
        .lifecycleMethod ("preUpgrade", async function (req, socket, head)
        {
            ...
            await cls.applyPlugins ("serverplugins", "preUpgrade", self, req, socket, head);
            await self.emit ("preUpgrade", self, req, socket, head);
        })
        .lifecycleMethod ("postUpgrade", async function (req, socket, head)
        {
            ...
            await cls.applyPlugins ("serverplugins", "postUpgrade", self, req, socket, head);
            await self.emit ("postUpgrade", self, req, socket, head);
        })
        ...
        .lifecycleMethod ("preStart", async function ()
        {
            ...
            await cls.applyPlugins ("serverplugins", "preStart", self);
            await self.emit ("preStart", self);
        })
        .lifecycleMethod ("postStart", async function ()
        {
            ...
            await cls.applyPlugins ("serverplugins", "postStart", self);
            await self.emit ("postStart", self);
        })
