module.exports = function (nit, http, Self)
{
    ...
    return (Self = nit.defineClass ("http.Server"))
        ...
        {
            Descriptor
                .field ("contextClass", "http.Context.Descriptor", "The server context class.",
                {
                    defval:
                    {
                        requestfilters:
                ...
                .field ("certificates...", "http.Certificate.Descriptor", "The SSL certificates.")

                .onConfigure (function (server)
                {
                    let self = this;

                    server.contextClass = self.contextClass.build ();
                    server.services.push (...self.services.map (s => s.build ()));
                    server.certificates.push (...self.certificates.map (c => c.build ()));
                })
            ;
        })
        ...
        .field ("certificates...", "http.Certificate", "The certificates to use.")
        .field ("contextClass", "function", "The context class to use.", () => Self.Descriptor ().contextClass.build ())
