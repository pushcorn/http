module.exports = function (nit, http, Self)
{
    ...
    return (Self = nit.defineClass ("http.Server"))
        ...
        .lifecycleMethod ("dispatch", async function (req, res)
        {
            ...
            await self.emit ("preDispatch", ctx);
            ...
            await self.emit ("postDispatch", ctx);
        ...
        .lifecycleMethod ("preInit")
        .lifecycleMethod ("postInit")
        ...
        .lifecycleMethod ("preUpgrade")
        .lifecycleMethod ("postUpgrade")
        .lifecycleMethod ("upgrade", async function (req, socket, head)
        {
            await Self.selectObjectForHost (this.hosts, req.hostname)?.upgrade (req, socket, head);
        })
        ...
        .lifecycleMethod ("start", async function ()
        {
            ...
            await self.preInit ();
            await self.emit ("preInit");
            await self.init ();
            await self.emit ("init");
            await self.postInit ();
            await self.emit ("postInit");

            await self.preStart ();
            await self.emit ("preStart");
