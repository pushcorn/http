module.exports = function (nit, http, Self)
{
    return (Self = nit.defineClass ("http.Server"))
        ...
        {
            Method
                .before ("preInit.invokeHook", "preInit.createNodeServer", (self) =>
                {
                    if (self.ssl)
                    {
                ...
                .after ("init.applyPlugins", "init.callHosts", async function (self)
                {
                ...
                .afterComplete ("returnServer", self => self)
            ;
        })
        .configureComponentMethod ("start", Method =>
        {
            Method
                .after ("preStart.stopIfStarted", "preStart.initServer", async function (self)
                {
                    await self.initialize ();
                })
                .after ("start", "start.startNodeServer", (self) => self.startNodeServer ())
                .after ("start.applyPlugins", "start.callHosts", async function (self)
                {
                    for (let host of self.hosts)
                    {
                ...
                .beforeFailure ("stopServer", function (self)
                {
        ...
        {
            Method
                .after ("preStop.stopIfStopped", "preStop.log", self => self.info ("info.stopping_server"))
                .after ("preStop.log", "preStop.closeServer", async function (self)
                {
                ...
                .after ("stop", "stop.callHosts", async function (self)
                {
                    for (let host of self.hosts)
                    {
                ...
                .before ("after", "postStop.log", self => self.info ("info.server_stopped"))
                .beforeFailure ("logError", function (self)
                {
        ...
        {
            Method
                .after ("upgrade.applyPlugins", "upgrade.callHost", (self, req, socket, head) => http.selectApplicableObject (self.hosts, req)?.upgrade (req, socket, head))
                .afterComplete ("returnServer", self => self)
            ;
        })
        ...
        {
            Method
                .before ("initArgs", async function (self, req, res)
                {
                ...
                .after ("initArgs", "selectHost", (self, ctx) => ctx.host = http.selectApplicableObject (self.hosts, ctx))
                .after ("dispatch.applyPlugins", "dispatch.callHost", async (self, ctx) => ctx.host?.dispatch (ctx))
                .afterSuccess ("checkResponse", (self, ctx) => ctx.response = ctx.response || http.responseFor (404))
                .beforeFailure ("castError", function (self, ctx)
                {
                ...
                .afterComplete ("writeResponse", async (self, ctx) =>
                {
                    try
                    {
