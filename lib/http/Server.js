module.exports = function (nit, http, Self)
{
    return (Self = nit.defineClass ("http.Server"))
        ...
        {
            let self = this;
            let defaultHost = nit.pick (self, "names", "services", "certificate");
        ...
        {
            queue
                .after ("preStop", "preStop.log", () => self.info ("info.stopping_server"))
                .before ("postStop", "postStop.log", () => self.info ("info.server_stopped"))
                .before ("stop.invokeHook", "stop.closeServer", async function ()
                {
                ...
                .replace ("stop.callHosts", async function ()
                {
                    try
                    {
                        for (let host of self.hosts)
        ...
        {
            let host = http.selectApplicableObject (self.hosts, req.hostname);
