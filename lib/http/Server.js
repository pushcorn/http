module.exports = function (nit, http, Self)
{
    return (Self = nit.defineClass ("http.Server"))
        ...
        {
            Queue
                .after (`${method}.applyPlugins`, `${method}.callHosts`, async function (self)
                {
                    for (let host of self.hosts)
                    {
                        await host[method] (...this.args);
                    }
                })
                .onComplete (self => self)
            ;
        })
        .configureComponentMethod ("stop", true, (Queue, method) =>
        {
            Queue
                .before (`${method}.invokeHook`, `${method}.callHosts`, async function (self)
                {
                    for (let host of self.hosts)
                    {
                        await host[method] (...this.args);
        ...
        {
            Queue
                .after ("preStop.returnIfStarted", "preStop.log", self => self.info ("info.stopping_server"))
                .before ("postStop", "postStop.log", self => self.info ("info.server_stopped"))
                .before ("stop.callHosts", "stop.closeServer", async function (self)
                {
        ...
        .configureComponentMethod ("upgrade", Queue =>
        {
            Queue
        ...
        .configureComponentMethod ("dispatch", Queue =>
        {
            Queue
                .onInit (function (self, req, res)
                {
                ...
                .after ("dispatch.applyPlugins", "dispatch.callHost", (self, ctx) => ctx.host?.dispatch (ctx))
                .onSuccess ((self, ctx) =>
                {
        ...
        {
            Queue
                .lpush ("init", async function (self)
                {
                    await self.initialize ();
                })
                .before ("preStart.invokeHook", "preStart.createNodeServer", (self) =>
                {
                    if (self.ssl)
                    {
                ...
                .before ("start.invokeHook", "start.startNodeServer", (self) => self.startNodeServer ())
                .onFailure (function (self)
                {
