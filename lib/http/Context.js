    ...
    return (Self = nit.defineClass ("http.Context"))
        ...
        .defineClassDescriptor (Descriptor =>
        {
            Descriptor
                ...
                .onConfigure (function (contextClass)
                {
                    let self = this;

                    self.requestfilters.forEach (f => contextClass.requestfilter (f.build ()));
                    self.responsefilters.forEach (f => contextClass.responsefilter (f.build ()));
                })
            ;
        })
        ...
        .property ("response", "http.Response", // The typed server response.
        {
            setter: function (resp)
            {
                if (resp != this.response)
                {
                    ...
                    let empty = Self.EMPTY_BODY_STATUSES.includes (respClass.status);

                    self.responseHeader ("X-Response-Name", respClass.simpleName);
        ...
        .staticMethod ("new", function (method, path, params)
        {
        ...
        .method ("buildRequest", async function (cls)
        {
            ...
            return cls.validate (new cls (params));
        })
