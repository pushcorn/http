module.exports = function (nit, http, Self)
{
    ...
    return (Self = nit.defineClass ("http.Context"))
        ...
        .field ("templateLoaders...", "http.TemplateLoader", "The template loaders.")
        ...
        .method ("resolveAsset", async function (path)
        {
            ...
            return await nit.find (self.assetResolvers, async (resolver) =>
                {
                    let f = await resolver.resolve (path);

                    return f && nit.find.Result (f);
                })
                || (await self.hanlder?.resolveAsset (path))
        ...
        .method ("loadTemplate", async function (path)
        {
            ...
            return await nit.find (self.templateLoaders, async (loader) =>
                {
                    let t = await loader.load (path, self);

                    return t && nit.find.Result (t);
                })
                || (await self.handler?.loadTemplate (path, self))
