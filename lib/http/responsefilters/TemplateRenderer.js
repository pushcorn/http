...
{
    return (Self = http.defineResponseFilter ("TemplateRenderer"))
        .use ("http.Etag")
        .use ("nit.utils.Crypto")
        .use ("http.responses.FileReturned")
        .use (["TemplateEntry", "http.cache.entries.Template"])
        .constant ("TMP_FILE_PREFIX", nit.uuid ().slice (0, 6))
        ...
        .onApply (async function (httpContext)
        {
            ...
            else
            {
                ...
                path = nit.path.join (nit.os.tmpdir (), `${Self.TMP_FILE_PREFIX}-${hash}.html`);
