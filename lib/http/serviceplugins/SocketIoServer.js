module.exports = function (nit, http, Self)
{
    ...
    return (Self = http.Service.defineServicePlugin ("SocketIoServer"))
        ...
        .defineInnerClass ("Manager", Manager =>
        {
            Manager
                ...
                .construct (function (service, path)
                {
                    ...
                    self.io = new http.SocketIo.Server (nodeServer, { path, serveClient: false });
                    ...
                    self.io.on ("connection", function (socket)
                    {
                        socket.on ("message", async function (method, path, data, cb)
                        {
                            ...
                            if (nit.is.func (cb))
                            {
                                cb ((ctx?.response || http.responseFor (500)).toPojo ()); // eslint-disable-line callback-return
                            }
                        });
                    });
        ...
        .method ("preInit", function (service)
        {
            let self = this;
        ...
        .method ("preUpgrade", function (service, req, socket, head)
        {
            service.socketIo.handleUpgrade (req, socket, head);
        })
        .method ("preDispatch", function (service, ctx)
        {
            service.socketIo.handleRequest (ctx);
        })
        .method ("preStop", function (service)
        {
