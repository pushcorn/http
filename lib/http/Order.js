    ...
    return (Self = nit.defineClass ("http.Order"))
        ...
        .staticMethod ("onRegisterPlugin", function (hostCls)
        {
            hostCls
                ...
                {
                    afters.forEach (after => this.order (new Self (this.name, Self.getTargetName (this.name, after))));

                    return this;
                })
                .staticMethod ("orderAfter", function (...befores)
                {
                    befores.forEach (before => this.order (new Self (Self.getTargetName (this.name, before), this.name)));
