module.exports = function (nit, Self)
{
    return (Self = nit.defineCommand ("http.commands.Server"))
        ...
        .defineMeta ("returnServer", "boolean", !nit.CLI_MODE, { enumerable: false })
        .constant ("DEFAULT_DESCRIPTOR",
        {
            plugins: "http:auto-restart",
            services:
            [
            {
        ...
        .onRun (async function ({ input })
        {
            ...
            if (!descriptor)
            {
                descriptor = nit.new ("http.Server.Descriptor", Self.DEFAULT_DESCRIPTOR);

                if (root)
                {
                    descriptor.services[0].assetResolvers = { roots: root };
                }
            }
