module.exports = function (nit, http, Self)
{
    return (Self = http.defineResponse ("http.responses.View"))
        ...
        .field ("<template>", "string|nit.Template|nit.File", "The view template.",
        {
            setter: function (v)
            {
                if (nit.is.str (v) && v.startsWith ("file://"))
                {
                    return new nit.File (v.slice (7));
                }
        ...
        .method ("toBody", async function (ctx)
        {
            ...
            if (template instanceof nit.File)
            {
                if ((file = await ctx.resolveAsset ("file://" + template.path)))
                {
