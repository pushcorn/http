module.exports = function (nit, Self)
{
    ...
    return (Self = nit.requireAsset ("public/lib/http"))
        ...
        .registerInnerClass ("IncomingMessage", nit.do (no_http.IncomingMessage, IncomingMessage =>
        {
            ...
            nit.memoize.dpg (IncomingMessage.prototype, "parsedUrl",
                function ()
                {
                    ...
                    ["pathname", "path", "href"].forEach (prop => parsed[prop] = ("/" + decodeURIComponent (nit.trim (parsed[prop]).replace (/\/$/, ""))).replace (/^\/\//, "/"));
        ...
        .staticMethod ("selectApplicableObject", function (objects, ctx)
        {
            if (!(ctx instanceof Self.Context))
            {
                if (nit.is.str (ctx))
                {
                    ctx = Self.Context.new ({ headers: { host: ctx } });
                }
                else
                if (ctx instanceof Self.IncomingMessage)
                {
                    ctx = new Self.Context (ctx, nit.new ("http.mocks.ServerResponse"));
                }
                else
                {
                    ctx = Self.Context.new (ctx);
                }
            }
