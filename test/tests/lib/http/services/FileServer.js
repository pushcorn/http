...
test.method ("http.services.FileServer", "init")
    .should ("use template filters if compileAssets is true")
        ...
        .expectingPropertyToBeOfType ("object.responseFilters.0", "http.responsefilters.TemplateRenderer")
        .expectingPropertyToBeOfType ("object.responseFilters.1", "http.responsefilters.TemplateRenderer")
        .commit ()

    .should ("not use template filters if compileAssets is false")
        .up (s => s.createArgs = false)
        .expectingPropertyToBe ("object.responseFilters.length", 0)
        .commit ()
;
